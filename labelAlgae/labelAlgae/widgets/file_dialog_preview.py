...
class FileDialogPreview(QtWidgets.QFileDialog):
    ...
    def onChange(self, path):
        if path.lower().endswith(".json"):
            ...
            #path450 = path.replace('bright', '450')
            #path638 = path.replace('bright', '638')

            #pixmap450 = QtGui.QPixmap(path450)
            #pixmap638 = QtGui.QPixmap(path638)
            ...
            if pixmap.isNull():
                ...
                self.labelPreview.label.setAlignment(QtCore.Qt.AlignTop)
                self.labelPreview.setHidden(False)
